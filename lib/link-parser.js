...
class LinkParser {
	 ...
	 * @returns {String}
	 *     Returns the parsed and transformed input.
	 */
	async render(input, linkTransform) {
		let output = '';
		for (const token of this.parse(input)) {
			if (token.type === 'link') {
				try {
					const link = await this.options.linkLookup(token);
 ...
 *     Returns a promise that resolves resolves with a modified link object.
 * @throws {Error}
